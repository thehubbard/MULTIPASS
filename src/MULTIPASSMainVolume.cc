//
// File:         MULTIPASS_Det/src/MULTIPASSMainVolume.cc
// Description:  MainVolume for MULTIPASS simulation. Based on original
...
MULTIPASSMainVolume::MULTIPASSMainVolume(G4RotationMatrix *pRot,
                 ...
                 "housing",pMotherLogical,pMany,pCopyNo),fConstructor(c)
{
  ...
  fScint_log = new G4LogicalVolume(fScint_box,G4Material::GetMaterial("EJ276"),
                                   "scint_log",0,0,0);

  // Always been aluminium even for PTFE diffuse like sims
  fHousing_log = new G4LogicalVolume(fHousing_box,
                             G4Material::GetMaterial("Al"), // aluminium foil!
                             "housing_log",0,0,0);
...
void MULTIPASSMainVolume::CopyValues(){
...
void MULTIPASSMainVolume::PlacePMTs(G4LogicalVolume* pmt_log,
...
void MULTIPASSMainVolume::VisAttributes(){
...
void MULTIPASSMainVolume::SurfaceProperties(){
  ...
  // new G4OpticalSurface("PTFE_HousingSurface", 
  //   unified, groundbackpainted, dielectric_dielectric);
  G4OpticalSurface * OpScintHousingSurface = 
  new G4OpticalSurface("ESR_HousingSurface", 
    unified, polishedbackpainted, dielectric_dielectric);

  OpScintHousingSurface->SetSigmaAlpha(1.3 * degree); // Janecek2010

  // From DETECT paper! - used to simulate air gap
  G4double refindex[] = {1,1};

  // SPECULAR SHOULDNT BE THERE FOR TEFLON!
  // Specular lobe is for specular reflector sucha s aluminium foil
  G4double specular_lobe[] = {1., 1.}; // Teflon R. Ogawara 
  //                                   // https://doi.org/10.1063/1.4959186
  scintHsngPT->AddProperty("SPECULARLOBECONSTANT", ephoton, specular_lobe,num);

  scintHsngPT->AddProperty("RINDEX", ephoton, refindex,num);
  // assumes well polised crystal
  ...
  new G4LogicalSkinSurface("esr_border_housing_surf",fHousing_log,
