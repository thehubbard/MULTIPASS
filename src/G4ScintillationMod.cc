//
// File:         MULTIPASS_Det/src/G4ScintillationMod.cc
// Description:  Modification of G4ScintillationMod 
//               Based on original code from Geant4 10.4.p01
// Version:      1.0.0
// Created:      2020-03-18
// Author:       Michael Hubbard
// Modified:     Changed to G4ScintillationMod
//               Modified PostStepDoIt method for multi time constant sim
//               Added the following methods for multi time constant sim:
//                   GetNumberOfTimeConstantsByParticleType
//                   GetNumberOfTCRatiosByParticleType
//                   CheckTCRatioEntries
//
//
// Tested with:
...

{
        ...
        G4MaterialPropertyVector* TC_ByType = 
                    GetNumberOfTimeConstantsByParticleType(aTrack);
        G4MaterialPropertyVector* Ratio_ByType = GetNumberOfTCRatiosByParticleType(aTrack);
        G4int nscntTCs = TC_ByType->GetVectorLength();
        // G4cout << "Did it work?: " << testtc << ", " << testrat << G4endl;
        CheckTCRatioEntries(nscntTCs,Ratio_ByType->GetVectorLength());
...
//
   ...
   // <<   "--  Mean Photons =  " << MeanNumberOfPhotons << "\n"
   // << G4endl;
        ...
        G4ScintillationType ScintillationType = Fast;

        // Currently only one emission profile configured
        ScintillationIntegral = (G4PhysicsOrderedFreeVector*)
                                      ((*fFastIntegralTable)(materialIndex));

        G4int scnt = 0;
        G4double stopSwitch = fNumPhotons * (*Ratio_ByType)[scnt];
        ScintillationTime = (*TC_ByType)[scnt];
        // G4cout << scnt << " "  << "Time constant: " << G4BestUnit(ScintillationTime,"Time") << "\n"
        // << "   Ratio: " << (*Ratio_ByType)[scnt] << "\n"
        //           << " Photons to sim: "<<Num<< G4endl;
        if (ScintillationIntegral){
            ...
            for (G4int i = 0; i < Num; i++) {
                if (i > stopSwitch) {
                  ++scnt;
                  if (scnt>=nscntTCs) {
                    G4cout << "Numnber of photons processed" <<stopSwitch <<"\n"
                    << "Total number of photons to be generated"<<fNumPhotons << G4endl;
                    G4ExceptionDescription ed;
                    ed << "\nG4Scintillation::PostStepDoIt(): "
                       << "Time constant out of range.\n"
                       << "Ratios by particle type likely do not add up to one.\n"
                       << G4endl;
                    G4String comments = "Issue MaterialPropertiesTable entry - Time constant ratios do not add to one.";
                    G4Exception("G4Scintillation::PostStepDoIt","Scint05",
                    FatalException,ed,comments);
                  }
                  stopSwitch += (fNumPhotons * (*Ratio_ByType)[scnt]);
                  ScintillationTime = (*TC_ByType)[scnt];
                  // G4cout << scnt << " "  << "Time constant: " << ScintillationTime << "\n"
                  // << "   Ratio: " << (*Ratio_ByType)[scnt] << G4endl;
                  // ScintillationType = scnt;
                }
...
G4MaterialPropertyVector* G4Scintillation::GetNumberOfTimeConstantsByParticleType(
...
G4MaterialPropertyVector* G4Scintillation::GetNumberOfTCRatiosByParticleType(
  ...
  if (!TC_Ratio_Vector) {
      ...
      ed << "\nG4Scintillation::PostStepDoIt(): "
         ...
         << G4endl;
      G4String comments = "Missing MaterialPropertiesTable entry - No correct entry in MaterialPropertiesTable";
      G4Exception("G4Scintillation::PostStepDoIt","Scint03",
      FatalException,ed,comments);
    }
